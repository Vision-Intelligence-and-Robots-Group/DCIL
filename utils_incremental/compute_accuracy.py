...
def compute_accuracy(tg_model, tg_feature_model, class_means, evalloader, scale=None, print_info=True, device=None, logger=None):
    if device is None:
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            outputs_feature = outputs_feature.cpu()
            outputs_feature = outputs_feature.numpy()
    ...
    if print_info:
        if logger:
            logger.info("  top 1 accuracy CNN            :\t\t{:.2f} %".format(100. * correct / total))
            logger.info("  top 1 accuracy iCaRL          :\t\t{:.2f} %".format(100. * correct_icarl / total))
            logger.info("  top 1 accuracy NCM            :\t\t{:.2f} %".format(100. * correct_ncm / total))
        else:
