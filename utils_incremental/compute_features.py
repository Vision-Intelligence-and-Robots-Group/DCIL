...
def compute_features(tg_feature_model, evalloader, num_samples, num_features, device):
    ...
    features = torch.from_numpy(features) # Tensor
    start_idx = 0
    with torch.no_grad():
        for inputs, targets in evalloader:
            inputs = inputs.to(device)
            features[start_idx:start_idx + inputs.shape[0], :] = torch.squeeze(tg_feature_model(inputs))
