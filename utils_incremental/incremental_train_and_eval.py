...
def incremental_train_and_eval_prox(args, epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                                     trainloader, testloader, \
                                     iteration, start_iteration, \
                                     lamda, last_model, round, mu, \
                                     dist, K, lw_mr, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if iteration == start_iteration:
                ...
                if ref_model != None and round > 0:
                    fedprox_reg = 0.0
                    for param_index, param in enumerate(tg_model.named_parameters()):
                        global_param_list = list(enumerate(last_model.named_parameters()))[param_index]
                        global_param = global_param_list[1]
                        fedprox_reg += torch.norm((mu / 2) * (param[1] - global_param[1]) ** 2)
                    loss += fedprox_reg
                ######## Implement FedProx Here #########
